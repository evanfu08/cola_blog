...
import { pushMediaFiles, type MediaUploads } from '../services/push-media'
...
export default function ConfigDialog({ open, onClose }: ConfigDialogProps) {
	...
	const [mediaUploads, setMediaUploads] = useState<MediaUploads>({})
	...
	const handleSave = async () => {
		...
		try {
			...
			if (mediaUploads.music || mediaUploads.video) {
				await pushMediaFiles(mediaUploads)
			}
	...
	return (
		<>
			...
			<DialogModal open={open} onClose={handleCancel} className='card scrollbar-none max-h-[90vh] min-h-[600px] w-[800px] overflow-y-auto'>
